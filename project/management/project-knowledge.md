...
**Status correction (2026-09-16)**: the "still a local feasibility prototype on Jura's machine, not deployed anywhere" framing from the 2026-09-07 portfolio review is now stale. Four internal BigHub documents (July-August 2026, not yet shared with Dr. Max) show the pipeline has actually progressed to validated, production-grade extraction — see below. Note: this internal engineering work is independent of the "API service specification" Radim mentioned receiving and beginning to test around 2026-09-14 (per the 2026-09-15 meeting) — the two are not confirmed to be the same artifact.

**Validated architecture & accuracy (BigHub internal, 2026-08)**: dual independent AI readings per page (GPT-5 primary + GPT-5-mini check, natively via Microsoft Azure AI Foundry, results merged), with Azure AI Document Intelligence added for deterministic checkbox detection on checklist-style forms (PBZ, door checklists). Any field where the two readings disagree, or where the system is unsure, is flagged for human review with a crop of the original attached. Measured on 292-370 real pages / 281 protocols across all 7 tuned document categories (automatic doors, PBZ/fire safety, electrical/EZS/lightning rods, air conditioning/HVAC, elevators, gas/pressure vessels/flue paths, fire extinguishers/hydrants): **91% accuracy on handwritten free-text content, 94% on structural form content**, 0 fabricated values found across 500+ pages tested, 14% of pages flagged for human review. Per-field accuracy on the 5 header fields ranges 76-100% (inspection date 92%/90.9%, address 76-86.8% — the biggest gap, attributed to an ambiguous definition of "address" rather than model quality, branch/cost-center 91.6-92%, protocol title and service company and verdict all measured near 100% but flagged as needing continued production monitoring). The production spec additionally tracks a **"silent error" rate** (wrong value with no review flag) per field — considered the single most important accuracy number, currently 1.4% (date) to 9% (address). Source: 2026-09-16-teo-ocr-accuracy-validation-cost-analysis, 2026-09-16-teo-ocr-production-spec-v1-1, illustrated concretely in 2026-09-16-teo-ocr-extraction-output-samples.

**Agreed production architecture (v1.1, 2026-08-07)**: blob storage intake → batch AI processing → API read-out. 7-step pipeline: (1) Dr. Max uploads to a `dropbox/` blob-storage folder, duplicate content is processed once; (2) each page converted to image (Word files read as text); (3) a smaller model classifies document category from page 1 (~95% accuracy), unknown types fall to a stricter-review general mode; (4) dual-model reading in batch mode (results within hours, guaranteed within 24h); (5) Document Intelligence cross-check for checkbox forms; (6) pages assembled into protocol records via deterministic (non-AI) rules — a new protocol only starts when both models agree on the boundary, disagreement flags it for human review; a protocol's final verdict is always the worst explicit verdict across its pages; (7) results read via API (`GET /ocr/protocols`, `/protocols/{id}`, `/protocols/{id}/render.pdf`), always addressed at protocol level since one file can bundle up to 50 protocols. Mailbox intake and ServiceNow write-back are explicitly out of scope for this phase, addable later without re-architecting — current output is API-readable structured records with crops/previews. Full technical API contract lives in a separate "OCR API v1.1" document (referenced but not yet ingested). Everything is replayable from the original `dropbox/` files (basis for audit/V&V). Source: 2026-09-16-teo-ocr-production-spec-v1-1.

**Cost (BigHub internal estimate)**: ~0.35 Kč/page in batch mode, ~0.55-0.60 Kč/page in faster/dual-inference mode; roughly 8,500-17,000 Kč/year (~$370-735) at an assumed 30,000 pages/year — no new paid Azure resources required, since GPT-5/GPT-5-mini are already deployed on the shared AI platform and storage/DB/monitoring already exist. Note: 30,000 pages/year sits close to, but not identical with, the ~250 docs/week × ~3 pages/doc (~39,000 pages/year) figure implied by the solution-proposal document — same order of magnitude, not yet reconciled to one authoritative number. Source: 2026-09-16-teo-ocr-accuracy-validation-cost-analysis.

**BigHub's proposed solution variants (internal draft, 2026-07, not yet shared with Dr. Max)**: four options with MD estimates — **Variant API** (12 MD, 4-6 week delivery): API on the existing AI platform, Dr. Max sends documents and receives structured JSON back, downstream SNOW import stays Dr. Max's responsibility; optional **+6 MD add-on** exposes per-category prompt editing to Dr. Max directly. **Variant HYBRID** (18 MD, targeted before the autumn service wave): full OCR pipeline triggered via API from Dr. Max's side, code-owned (not user-editable) prompts/categorization, tuned to ≥94% reliability on the 3 most common variants per category and ≥75% on others — reusable as the technical base for the Application variant. **Variant APLIKACE** (25-35 MD, also targeted before autumn service): a full internal validation web app — ingests from a shared mailbox, flags uncertain values with a reason, lets technical staff validate/correct in-app before manual export to ServiceNow; two further optional add-ons (TBD effort, dependent on Dr. Max's SNOW environment): automated SNOW write-back, and periodic SNOW read-out to cross-check inspection due-dates against submitted reports. BigHub recommends starting with the 3-4 highest-volume document categories to front-load ROI and validate accuracy before expanding. Which variant (if any) gets formally proposed to Dr. Max is not yet decided — see [[ASM-077]]. Source: 2026-09-16-teo-ocr-solution-proposal-variants.

**Provisioning for full automation** (not yet requested): three external grants needed before mailbox intake / SNOW write-back can go live — shared mailbox + Microsoft Graph API access (Dr. Max IT/M365 team, historically weeks for this type of cross-team request), AKS firewall egress rule (infra team, days to weeks), ServiceNow import-set API access (SNOW team, only needed for the write-back phase). Recommendation is to submit these in parallel, immediately once a phase is approved. Source: 2026-09-16-teo-ocr-accuracy-validation-cost-analysis.

**Blind real-world validation (2026-09-16 technical sync)**: BigHub ran the pipeline, untouched, against a real 2025 dataset (140 protocols across the 2 current pilot vendors — service-door suppliers). Measured accuracy came in a few percentage points above the 91%/94% benchmarks (not guaranteed to hold — may partly be luck per Jura). Device type, order number ("zakázka"), and service company all hit 100%. ~47/140 protocols (~a third) were flagged for human review, though several flags turned out to be false positives on data that was actually unambiguous — the real review burden may run lower than the raw flag rate suggests. One flag correctly caught a genuine BigHub-side data-entry error (date mistranscribed during setup), confirming the review mechanism catches real mistakes, not just noise. Branch-count reconciliation was close but imperfect (376 extracted vs. 356 actual, not investigated further). Two new bugs surfaced during review: the defects/findings free-text field is being duplicated in output, and one protocol (#136) had a genuine date error that should have been, but wasn't, flagged for review — a real instance of the "silent error" failure mode the production spec's accuracy tables are designed to track. Bounding-box crop alignment also has known imprecision in places (e.g. one EPS-reference crop not quite capturing the relevant text). Source: 2026-09-16-teo-ocr-technical-sync-pilot-results.

**Delivery progress (2026-09-16)**: BigHub is migrating its platform project to new AKS node pools with adequate compute, and will self-provision Blob storage for a TEST environment on the existing platform — free, self-service, with no BDC/infra-team (Vladislav Tvarůžek, STK-016) dependency to create it; the only possible infra ask is a single simple network-access grant if Radim Švarc needs external programmatic access. TEST (not yet production) is built to accept flexible real-world input (zip archives with nested folders, PDFs, images) rather than a pre-cleaned format, with an eye toward later supporting shared-mailbox intake without a rebuild. Tomáš Burda reviewed BigHub's technical/API documentation, found it in good shape, and is starting Dr. Max-side build work 2026-09-17, targeting something concrete by ~2026-09-23 (next joint sync). Vendor/service-company name extraction is confirmed constrained to literal source text (not normalized) specifically so it matches Dr. Max's supplier master list for ServiceNow import. Multi-branch/multi-protocol document splitting is confirmed working as designed: one document covering many branches splits into one row per branch; one document covering 2 devices at the same branch stays a single row with an equipment-count column. On Dr. Max's side, the ServiceNow-import form (built around "Wágner's," STK-047, BDC-side mechanism) is nearly complete — tested, bugs fixed except one cosmetic chart-rendering issue — expected live this week or early next. A possible 3rd, larger autumn-scope vendor ("PEDOS," name uncertain) was floated but not decided. Source: 2026-09-16-teo-ocr-technical-sync-pilot-results.

**Delivery progress (2026-09-22)**: "PEDOS" is resolved — the real vendor is **SPEDOS**, now being actively onboarded as the 3rd autumn-2026 pilot vendor (BigHub giving it extra pipeline attention, export due end of week — see [[ASM-127]]). Radim Švarc has built out the proposed intake/review architecture concretely on Dr. Max's side: a live mailbox (`automat.revize@drmax.cz`) and folder structure (`914 Technicke/revize/automat/`); a twice-weekly Power Automate flow pushing collected documents to BigHub's Blob storage (Radim has an open question back to BigHub on its batch-creation logic); a daily local Power Automate Desktop + Python script on his machine watching for BigHub's processed output batches (tracked via a local CSV), which builds a per-batch Excel (`{batchID}_processed_at.xlsx`) and renames/moves related PDFs into a "čeká" (waiting) folder; Michaela "Míša" Albrechtová manually reviews and corrects that Excel (same structure as Jura's template, with a clickable per-row link opening the source PDF at the exact page), after which the file moves to a "hotovo" (done) folder; a further Power Automate trigger then trims the Excel to SNOW's required columns, creates an import copy, and archives the source PDFs; ServiceNow import itself is manual via form today, with automatic import a real but not-yet-built possibility. Production output was also confirmed as an Excel file rather than the previously-discussed curated web interface. Radim's team will send "correct" excel/json corrections back to BigHub (format to be agreed with Jura) — the concrete implementation of [[ASM-120]]'s correction-feedback loop. Two extraction-scope decisions: reading from a stamp/seal is out of scope, and sending multiple disambiguation candidates to reviewers was rejected in favor of a simple manual-review flag. Address-matching gaps in the vendor directory (číselník) surfaced (e.g. "Bezručova" in Mělník not found) — TEO will verify it and push vendors toward complete addresses going forward. BigHub committed to a max-3-day processing turnaround.
...
A 6th type, **záznam teploměru** (temperature-log record), is also modeled in the Axapta contract and PDF confirmation output, but is not yet reachable from an actual scan — see [[ASM-134]].

**AR pairing key (2026-09-23, code audit)**: the spec's Závislosti section (per P. Sláma) says ZOPV and OPIATY still lack the AR pairing key and it's "promised" — but a direct code audit found the field already implemented and functional on both types. See [[ASM-133]] — treat the "still missing" framing as unverified until confirmed with P. Sláma.

**Document versioning — conflicting accounts (updated 2026-09-23)**: the 2026-09-16 conclusion below (multi-scan/historical-version storage out of scope for BigHub, Axapta owns it) is contradicted by a 2026-09-23 code audit, which found the app actively retains a full local version history per document (no row deleted, each re-scan gets its own versioned blob, version number computed locally and forwarded to Axapta). See [[ASM-132]] — which account is the target architecture is unresolved.

Original 2026-09-16 note (superseded pending reconciliation): multi-scan/historical-version storage is explicitly out of scope on BigHub's side — Axapta owns document versioning. BigHub only forwards new scans to Axapta with a "new version" flag; no local version history is maintained. Access-rights and read/write storage verification (tested with Jan Kopecký, STK-032) are done and confirmed to apply identically to both initiatives, since they share the same Axapta-backed access model. Source: 2026-09-16-devops-kanban-rollout-status-sync.

**Kiosk authentication (2026-09-23, code audit)**: the kiosk is treated throughout the spec and in client communication as fully anonymous/open-access (no login) — but the code requires Entra ID login across the entire scan flow, not just the PDF-confirmation endpoint. See [[ASM-131]] — this was communicated to the client (Jan Žižka) as open-access on 2026-09-10, so needs reconciling before the 2026-09-17 authentication debate.

**Multi-vehicle routes (2026-09-23, code audit)**: routes split across multiple vehicles (more than one ZOPV per AR) are treated in the spec as an open design question — but the code already has concrete behavior: a second vehicle's ZOPV silently overwrites the first as a new version of the same document. See [[ASM-135]].
...
**Business value — superseded (2026-09-23)**: a more detailed Business Quantification workshop breakdown (owner Jan Žižka, record dated 2026-08-01, status "K doplnění" — not yet formally approved) quantifies the saving as 10.5 h/day across 3 warehouses — Expedice Ostrava 1.5 h/den (1 pracovník), Xdock Brno 1 h/den (1 pracovník), Doprava Pavlov 4+2+2 h/den (3 pracovníci) — at a 50 000 Kč/month rate, ≈ **787 500 Kč/rok**, covering Fáze 1 and 2 combined. This should replace the Fermi estimate below once formally confirmed — see [[ASM-136]].

**Business value (Fermi estimate, flagged as possibly too large by the client herself — superseded, see above)**: ~16 hours/day (2 people × 8 hours) → a potential annual saving in the range of ~1.5 million (currency not stated). Source figure: an earlier Ableneo estimate of ~90 hours/month of relevant manual work, 40–60% considered automatable. Needs verification with Jan Žižka before being treated as reliable.
...
**KPIs — concrete targets added (2026-09-23, same workshop, status "K doplnění")**: Fáze 1 ≥40% of cases processed fully without manual intervention; Fáze 2 ≥40% of shipments with auto-verified km (ZOPV vs. Axapta vs. GPS Dozor); overall administrative time-fund reduced ~40% (aggregate, not per-headcount); ≥60% of temperature records auto-checked without manual reading. See [[ASM-136]].

**KPIs (prior candidate metric)**: Primary — total administrative time-fund reduced by ~40%, measured in aggregate (not per-headcount, since staff time isn't cleanly separable by task). Document error rate was explicitly discussed and rejected as a KPI, same reasoning as Reklamace — see [[ASM-072]].
...
**Implementation landed (2026-09-16)**: rolled out as an Azure DevOps board — one Epic per project (Fakturace doprav, Reklamace, order prediction, etc.), Stories underneath define business requirements ("as a user I click X and it does ABC"), Tasks are the dev-level breakdown. Explicitly not a full Scrum adoption — no fixed sprint cadence or time-boxing shown; **Easy Project remains the system of record for time/billing tracking**, the DevOps board exists purely for shared status visibility. Story-to-task granularity is currently inconsistent (some stories broken into very small tasks) — Jindřich's stance is to let this smooth out over time rather than standardize upfront. Jindřich also noted an AI agent can interact with the board programmatically via Azure CLI. Source: 2026-09-16-devops-kanban-rollout-status-sync.
